...
export const ColorPicker = ({
  ...
  // Notify parent of changes (only after initialization to avoid calling onChange on mount)
  useEffect(() => {
    ...
    if (mode === "hex") {
      colorString = alpha < 100 ? color.hexa() : color.hex();
    } else if (mode === "rgb" || mode === "css") {
      const rgb = color.rgb().array();
      colorString = `rgba(${Math.round(rgb[0])}, ${Math.round(rgb[1])}, ${Math.round(rgb[2])}, ${alpha / 100})`;
    } else if (mode === "hsl") {
      const hsl = color.hsl().array();
      colorString = `hsla(${Math.round(hsl[0])}, ${Math.round(hsl[1])}%, ${Math.round(hsl[2])}%, ${alpha / 100})`;
    } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute h-full rounded-full bg-transparent" />
      </div>
    </Slider>
  );
};
